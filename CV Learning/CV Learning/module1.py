...
def create_ytrue_train(img, labels, anchor_boxes, iou_upper = 0.7, iou_lower = 0.3):
    ...
    for label in labels:
        ...
        gt_bbox = [gtclass, gtx, gty, gtw, gth]
    ...
    for i,gt_box in enumerate(gt_box_arr):
        for j,anchor in enumerate(anchor_flat):
            if (anchor[2] == 0):
                ...
                iou_score[i,j] = iou(anchor, gt_box[1:],img_arr.shape[1],img.shape[0])
...
# Malisiewicz et al.
def non_max_suppression_fast(boxes,iou, overlapThresh=0.5):
...
#x = non_max_suppression_fast(flat_xyxy, iou_scores[0])
#print(x)
#flat_nms = np.array([flat[i] if i in x else [0,0,0,0] for i in range(len(flat))])
...
#x =np.array(range(flat.shape[0])) 
##for score in iou_scores:
##    x.append( np.array(non_max_suppression_fast(flat_xyxy, score, overlapThresh=0.5)))
##x = np.array(x)
##x_unique = np.unique(x)

#flat, original_shape = preprocess_anchor_boxes(generate_anchor_boxes(112,112,16,scale=[8,16,32], no_exceed_bound = True))
#flat_xyxy = np.array([xywh_to_xyxy(box) for box in flat])

#iou_scores = create_ytrue_train(img=np.zeros((112,112)), labels=[[1,40,40,20,10],[2,20,20,20,20]], anchor_boxes = flat)
